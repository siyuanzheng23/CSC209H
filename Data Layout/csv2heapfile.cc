...
int main(int argc, char ** argv) {
	if (argc != 4) {
		fprintf(stderr,
				"Usage: csv2heapfile <csv_file> <heapfile> <page_size>\n");
		exit(1);
	}

	char *csv_name = argv[1];
	char *heap_name = argv[2];
	...
	FILE* heap_file = fopen(heap_name, "w");
	if (heap_file == 0) {
		fprintf(stderr, "Could not open file %s\n", heap_name);
		exit(1);
	}
	...
	while(getline(csv_file, line)){
		...
		if (fixed_len_page_freeslots(&page) == 0) {
			...
			fwrite(page.data, page_size,1, heap_file);
	fflush(heap_file);
			page_num++;
	...
	if(usedslot>0){
		fwrite(page.data, 1, usedslot * page.slot_size, heap_file);
		fflush(heap_file);
		page_num++;
	}
